...
def memory_manager(q_new_batch, reps_x, reps_y, reps_w, lock, lock_make, lock_made, num_classes, num_candidates, num_representatives, batch_size, cuda, device):
...
class nil_v1_agent(Agent):
    ...
    def before_all_tasks(self, train_data_regime):
        ...
        self.p = mp.Process(target=memory_manager, args=[self.q_new_batch, self.reps_x, self.reps_y, self.reps_w, self.lock, self.lock_make, self.lock_made, self.model.num_classes, self.num_candidates, self.memory_size, self.batch_size, self.cuda, hvd.local_rank()])
        self.p.start()
